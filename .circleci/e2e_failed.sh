...
failed_diff_images=$(cat mochawesome.json \
  | jq -r '.suites.suites[].tests[]|select(.state=="failed" and (.err|has("message")) and (.err.message|match("images are different")))|(.context|fromjson.value.filename)+"/"+(.err.message|gsub("Error: ";"")|gsub(" images are different";"")+"-diff.png")' \
